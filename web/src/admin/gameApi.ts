...
export const createGame = (packId: string, title: string, teamCount: number, answerTimerSec: number, finalAnswerTimerSec: number): Promise<{ gameId: string }> =>
  fetch('/api/games', { method: 'POST', headers: jsonHeaders, body: JSON.stringify({ packId, title, teamCount, answerTimerSec, finalAnswerTimerSec }) }).then(jsonOf);
