...
async function seedGame(opts: { tag: string; normalType: 'text' | 'audio'; withFinal: boolean }): Promise<{ cookie: string; gameId: string }> {
  const { tag, normalType, withFinal } = opts;
  ...
  const f1 = withFinal ? await makeBankQuestion(cookie, categoryId, 'text', `Финал-1 ${tag}`) : null;
  const f2 = withFinal ? await makeBankQuestion(cookie, categoryId, 'text', `Финал-2 ${tag}`) : null;
